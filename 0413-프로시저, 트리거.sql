...
-- 1-3) 입력 파라미터(IN)
-- 프로시저에 전달하는 값 : 인수
-- 예제 : employee_id를 입력 파라미터로 전달하면 해당 사원의 last_name 출력하기
CREATE OR REPLACE PROCEDURE proc3(in_employee_id IN NUMBER)  -- 타입 작성 시 크기 지정 안 함(에러 남)
IS
...
DECLARE
    max_salary NUMBER;
BEGIN
    proc4(max_salary);  -- max_salary에 최고연봉이 저장
...
BEGIN
    ...
    ELSIF UPDATING THEN -- 갱신했다면
        DBMS_OUTPUT.PUT_LINE('UPDATE 했군요');
    ELSIF DELETING THEN -- 갱신했다면
...
-- 문제
-- 사원(employees) 테이블에서 삭제된 데이터는 퇴사자(retire) 테이블에 자동으로 저장되는 트리거를 작성하시오
