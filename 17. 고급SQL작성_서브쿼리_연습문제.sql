...
-- 스칼라 서브쿼리는 성능이 좋지 않아 조인으로 풀 수 있다면 조인으로!!


-- 4. 평균연봉 이상의 연봉을 받는 사원들의 정보(last_name, salary)를 조회하시오.
SELECT last_name
         , salary
 FROM employees
...
-- first_name : Patrick
-- last_name : Sully
-- 서브쿼리 : Patrick Sully의 부서를 알아야 한다(다중 행 서브쿼리)
SELECT  first_name
          , last_name
          , department_id
  FROM employees
...
-- 6. 부서번호가 20인 사원들 중에서 평균연봉 이상의 연봉을 받는 사원정보(last_name, salary, department_id)를 조회하시오.
SELECT  last_name
          , salary
          , department_id
  FROM employees
...
-- 7. 직업(job_id)이 'PU_MAN'의 최대연봉보다 더 많은 연봉을 받은 사원들의 정보(last_name, salary, job_id)를 조회하시오.
-- 1) 다중 행 서브쿼리
SELECT  last_name
          , salary
          , job_id
...
-- 8. 사원번호가 131인 사원의 JOB_ID와 SALARY가 모두 일치하는 사원들의 정보(last_name, job_id, salary)를 조회하시오.
SELECT last_name
         , job_id
         , salary
 FROM employees
...
-- 12. MANAGER가 아닌 사원들의 정보(employee_id, last_name)를 조회하시오.
...
-- 13. 근무지가(city) 'Southlake'인 사원들의 정보(employee_id, last_name)를 조회하시오.
-- 서브쿼리1 : 근무지가 'Southlake'인 location_id를 locations 테이블에서 조회
...
-- 14. 부서명의 가나다순으로 모든 사원의 정보(employee_id, last_name를 조회하시오.
SELECT employee_id
         , last_name
 FROM employees e
